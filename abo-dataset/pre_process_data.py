...
if __name__ == "__main__":
    ...
    # First convert to pandas to use sklearn's train_test_split
    indices = np.array(range(len(preprocessed_df)))
    ...
    # Split the DataFrame and embeddings
    train_df = preprocessed_df.filter(pl.lit(train_mask.tolist()))
    val_df = preprocessed_df.filter(pl.lit(val_mask.tolist()))
    test_df = preprocessed_df.filter(pl.lit(test_mask.tolist()))
